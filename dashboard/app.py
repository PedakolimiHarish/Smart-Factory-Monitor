...
def on_message(client, userdata, message):
    ...
    try:
        ...
        with lock:
            ...
            userdata["history"]["temperature"].append( data["temperature"])
            userdata["history"]["vibration"].append( data["vibration"])
            userdata["history"]["rpm"].append(data["rpm"])
            userdata["history"]["current"].append(data["current"])
...
with st.session_state.data_lock:
    ...
    history = { key: list(values)for key, values in st.session_state.history.items()}

st.title("🏭 Smart Factory Monitor")
st.caption( "Industry 4.0 — Industrial IoT Machine Monitoring")
...
with col1:
    st.metric("Temperature", f"{data['temperature']:.2f} °C")

with col2:
    st.metric( "Vibration", f"{data['vibration']:.2f} mm/s")

with col3:
    st.metric("RPM", f"{data['rpm']:.0f}")
...
if len(history["temperature"]) > 0:
    ...
    st.line_chart( df["vibration"], height=250)
    st.caption("Vibration (mm/s)")

    # RPM
    st.write("### ⚙️ Motor Speed")
    st.line_chart( df["rpm"], height=250)
    st.caption("Speed (RPM)")

    # Current
    st.write("### ⚡ Motor Current")
    st.line_chart( df["current"], height=250)
